...
{
	// Calling the view function to retrieve the arrays
    let res = await contractInstance.methods.getResults().call();
    ...
    // Only print if there was at least one match
    if(buyAddrs && buyAddrs.length > 0) {
        console.log('index\t sellAddresses\t\t\t\t\t buyAddresses\t\t\t\t\t C\t Q');
        for(let i = 0; i < buyAddrs.length; i++) {
            console.log(`${i+1}\t ${sellAddrs[i]}\t ${buyAddrs[i]}\t ${C}\t ${qs[i]}`);
        }
    }
}
