...
{
  try {
      await contractInstance.methods.addSeller(quantity, value).send({from: fromAddress, gasLimit: '0xe00000'});
      console.log('submitted a sell bid of: (' + quantity + ', ' + value + ') from account: ' + fromAddress);
  } catch (error) {
      console.log('Error submitting sell bid:', error.message, 'Reverted for account: ', fromAddress);
  }
