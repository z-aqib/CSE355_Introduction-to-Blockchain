...
import "../contracts/banking_29008_contract.sol";
...
contract BankingTest
{
    ...
    // User 2 created an account
    /// #sender: account-2 (sender is account at index '2')
    function openAcc2() public 
    {
        TestSystem.openAccount("Rohaim","Imran");
    }
    ...
    /// #sender: account-1 (sender is account at index '1')
    function transferingToNonExistentAccount() public 
    {
        address payable userP3 = payable(user3);
        try TestSystem.TransferEth(userP3,1000000000000000000) 
        {
            conter = 1;
        }
        catch Error(string memory reason)
        {
            Assert.equal(reason,string("Recipient account does not exist"),"It is not giving the right error message");
        }
        Assert.equal(conter, 0,"It is allowing the user to transfer to a non existent account.");     
    }
    ...
    function CheckingLoanTransferred() public 
    {
        ...
        Assert.equal(principal, uint(BalanceAfterLoan-originalBalance), "Wrong Balance");   
    }
    ...
    // USER 1 tries to pay back less - it should not return any error.
    /// #sender: account-1 (sender is account at index '1') 
    /// #value: 5000000000000000000
    function TryingToReturnLessThanLoanAmount() public 
    { 
        TestSystem.returnLoan{value:500000000000000000}();
        (uint principal, uint interest, uint total) = TestSystem.InquireLoan();
        Assert.equal(principal, uint(500000000000000000), "Wrong Loan Record");   
    }

    uint public c = 0;
    /// #sender: account-3 (sender is account at index '2')
    ...
    function UserWithNoAccountReturningLoan() public payable  
    {
        ...
        Assert.equal(contra, 0,"This should revert!");     
    }

    uint public kon = 0;
    /// #sender: account-3 (sender is account at index '3')
    function UserWithNoAccountTakingLoan() public
    {
        kon = 0;
        try TestSystem.TakeLoan(1000000000000000000) 
        {
        ...
        Assert.equal(kon, 0,"This should revert!");
    }
